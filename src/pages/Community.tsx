...
import { MessageSquare, ThumbsUp, CheckCircle, Shield, AlertCircle, Users, Search, Filter, BookOpen, Heart, MessageCircle, Plus, X, Eye, EyeOff, AlertTriangle, Send, ArrowLeft, Trash2, Settings, Lock, Loader2, RefreshCw, Edit2, Flag } from "lucide-react";
...
NPC:`;
  ...
  const handleDeletePost = (postId: number, isPatient: boolean) => {
    if (window.confirm("Bạn có chắc chắn muốn xóa bài đăng này?")) {
      if (isPatient) {
        setPatientPosts(posts => posts.filter(p => p.id !== postId));
      } else {
        setAcademicPosts(posts => posts.filter(p => p.id !== postId));
      }
    }
  };

  const handleEditPost = (postId: number, isPatient: boolean) => {
    if (isPatient) {
      setPatientPosts(posts => posts.map(p => p.id === postId ? { ...p, content: editPostContent } : p));
    } else {
      setAcademicPosts(posts => posts.map(p => p.id === postId ? { ...p, content: editPostContent } : p));
    }
    setEditingPostId(null);
  };

  const handleDeleteComment = (postId: number, commentId: number, isPatient: boolean) => {
    if (window.confirm("Xóa bình luận này?")) {
      if (isPatient) {
        setPatientPosts(posts => posts.map(p => p.id === postId ? { ...p, comments: p.comments.filter(c => c.id !== commentId) } : p));
      } else {
        setAcademicPosts(posts => posts.map(p => p.id === postId ? { ...p, comments: p.comments.filter(c => c.id !== commentId) } : p));
      }
    }
  };

  const handleReport = (type: 'post' | 'comment') => {
    alert(`Đã gửi báo cáo vi phạm ${type === 'post' ? 'bài đăng' : 'bình luận'} tới quản trị viên.`);
  };
  ...
  const filteredChatRooms = chatRooms.filter(room => {
    if (roomSearchQuery && !room.name.toLowerCase().includes(roomSearchQuery.toLowerCase()) && !room.id.toString().includes(roomSearchQuery)) return false;
    return true;
  });
                    ...
                    <div className="flex-1">
                      ...
                      {editingPostId === thread.id ? (
                        <div className="mb-3" onClick={(e) => e.stopPropagation()}>
                          <textarea 
                            ...
                            className="w-full p-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500"
                            rows={3}
                          />
                          <div className="flex space-x-2 mt-2">
                            <button onClick={() => handleEditPost(thread.id, false)} className="px-3 py-1 bg-emerald-600 text-white text-xs rounded-md hover:bg-emerald-700">Lưu</button>
                            <button onClick={() => setEditingPostId(null)} className="px-3 py-1 bg-slate-200 text-slate-700 text-xs rounded-md hover:bg-slate-300">Hủy</button>
                          </div>
                        </div>
                      ) : (
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex space-x-2" onClick={(e) => e.stopPropagation()}>
                          {thread.author === "Young Guru (Bạn)" ? (
                            <>
                              <button onClick={() => { setEditingPostId(thread.id); setEditPostContent(thread.content); }} className="p-1.5 text-slate-400 hover:text-emerald-600 rounded-lg" title="Chỉnh sửa"><Edit2 className="w-4 h-4" /></button>
                              <button onClick={() => handleDeletePost(thread.id, false)} className="p-1.5 text-slate-400 hover:text-rose-600 rounded-lg" title="Xóa bài đăng"><Trash2 className="w-4 h-4" /></button>
                            </>
                          ) : (
                            <button onClick={() => handleReport('post')} className="p-1.5 text-slate-400 hover:text-amber-600 rounded-lg" title="Tố cáo bài đăng"><Flag className="w-4 h-4" /></button>
                          )}
                        </div>
                      </div>
                    </div>
                        ...
                        <div key={comment.id} className={cn("p-4 rounded-xl flex items-start space-x-3", comment.isPeerReviewed ? "bg-blue-50/50 border border-blue-100" : "bg-white border border-slate-100")}>
                          ...
                          <div className="flex-1">
                            ...
                            <div className="mt-2 flex items-center space-x-4">
                              ...
                              {comment.author === "Young Guru (Bạn)" ? (
                                <button onClick={() => handleDeleteComment(thread.id, comment.id, false)} className="text-xs font-medium text-slate-400 hover:text-rose-600">Xóa</button>
                              ) : (
                                <button onClick={() => handleReport('comment')} className="text-xs font-medium text-slate-400 hover:text-amber-600">Tố cáo</button>
                              )}
                            </div>
                          </div>
                        </div>
                    ...
                    <div>
                      ...
                      {editingPostId === post.id ? (
                        <div className="mb-3" onClick={(e) => e.stopPropagation()}>
                          <textarea 
                            value={editPostContent} 
                            onChange={e => setEditPostContent(e.target.value)}
                            className="w-full p-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-rose-500/20 focus:border-rose-500"
                            rows={3}
                          />
                          <div className="flex space-x-2 mt-2">
                            <button onClick={() => handleEditPost(post.id, true)} className="px-3 py-1 bg-rose-600 text-white text-xs rounded-md hover:bg-rose-700">Lưu</button>
                            <button onClick={() => setEditingPostId(null)} className="px-3 py-1 bg-slate-200 text-slate-700 text-xs rounded-md hover:bg-slate-300">Hủy</button>
                          </div>
                        </div>
                      ) : (
                      ...
                      <div className="mt-3 flex items-center justify-between text-slate-500">
                        ...
                        <div className="flex space-x-2 items-center">
                          ...
                          {post.author === "Young Guru (Bạn)" ? (
                            <>
                              <button onClick={() => { setEditingPostId(post.id); setEditPostContent(post.content); }} className="p-1 text-slate-400 hover:text-rose-600 rounded-lg ml-2" title="Chỉnh sửa"><Edit2 className="w-3.5 h-3.5" /></button>
                              <button onClick={() => handleDeletePost(post.id, true)} className="p-1 text-slate-400 hover:text-rose-600 rounded-lg" title="Xóa bài đăng"><Trash2 className="w-3.5 h-3.5" /></button>
                            </>
                          ) : (
                            <button onClick={() => handleReport('post')} className="p-1 text-slate-400 hover:text-amber-600 rounded-lg ml-2" title="Tố cáo bài đăng"><Flag className="w-3.5 h-3.5" /></button>
                          )}
                        </div>
                      </div>
                          ...
                          {post.comments.map(comment => (
                            <div key={comment.id} className="bg-white p-3 rounded-xl border border-slate-100">
                              <div className="flex justify-between items-start">
                                ...
                                {comment.author === "Young Guru (Bạn)" ? (
                                  <button onClick={() => handleDeleteComment(post.id, comment.id, true)} className="text-[10px] font-medium text-slate-400 hover:text-rose-600">Xóa</button>
                                ) : (
                                  <button onClick={() => handleReport('comment')} className="text-[10px] font-medium text-slate-400 hover:text-amber-600">Tố cáo</button>
                                )}
                              </div>
              ...
              <div className="p-6 overflow-y-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {filteredChatRooms.length === 0 ? (
                    <div className="col-span-full text-center py-10 text-slate-500">Không tìm thấy phòng nào phù hợp.</div>
                    ...
                    <div key={room.id} onClick={() => handleJoinRoom(room)} className="p-4 bg-white border border-slate-200 rounded-2xl hover:border-emerald-300 hover:shadow-sm transition-all cursor-pointer group relative">
                      ...
                      <div className="flex justify-between items-start mb-3 pr-8">
                        <h4 className="font-semibold text-slate-900 group-hover:text-emerald-700 transition-colors flex items-center">
                          ...
                          <span className="text-xs text-slate-400 font-mono mr-2">#{room.id}</span>
                          {room.name}
                        </h4>
